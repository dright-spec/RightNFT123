...
export function AutoNFTMinter({ rightId, onMintComplete }: AutoNFTMinterProps) {
  ...
  const recordNFTMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The right has been tokenized as an NFT on Hedera blockchain.",
      });
    },
    onError: (error) => {
  ...
  useEffect(() => {
    const attemptNFTMint = async () => {
      if (!right || right.verificationStatus !== "verified" || right.hederaTokenId || isMinting) {
        return;
      }
      ...
      try {
        toast({
          title: "Minting NFT",
          description: "Creating NFT on Hedera blockchain for verified right...",
        });
        ...
        await recordNFTMutation.mutateAsync({
          ...
          accountId: "0.0.123456", // Would be actual user's account
          network: "testnet",
        });
        ...
        toast({
          ...
          description: "Failed to mint NFT on Hedera blockchain. Please try again.",
          variant: "destructive",
        });
      } finally {
        setIsMinting(false);
    ...
    attemptNFTMint();
  }, [right?.verificationStatus, right?.hederaTokenId, rightId, isMinting]);
