...
## Changelog
- June 26, 2025: **HASHCONNECT V3 MIGRATION COMPLETED**: Migrated entire platform to HashConnect v3 API following official documentation. Replaced all v2 HashConnect references with proper v3 syntax including correct constructor parameters, event listeners (.on instead of .subscribe), and wallet discovery methods (findLocalWallets). Eliminated all legacy v2 code that was causing "Invalid encrypted text received. Decryption halted" errors. Platform now uses authentic HashConnect v3 API for reliable HashPack wallet connections.
