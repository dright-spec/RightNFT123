...
## Changelog
- June 25, 2025: **SIMPLIFIED DIRECT HASHPACK CONNECTION**: Fixed broken wallet connection by implementing simple direct HashPack API calls. Removed complex connector classes that were causing "this.hashConnect.on is not a function" errors. Uses straightforward window.hashpack.requestAccountInfo() with proper error handling and installation detection. Provides clear user guidance when HashPack isn't installed with direct Chrome Web Store links. Eliminates all unnecessary abstraction layers.
