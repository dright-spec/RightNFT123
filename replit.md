...
## Changelog
- June 26, 2025: **RESTORED WORKING DIRECT HASHPACK API**: Returned to the successful direct HashPack API integration that bypasses HashConnect entirely. Uses client/src/lib/hashpack-direct.ts which connects directly to window.hashpack extension API without any HashConnect middleware. This eliminates all "Invalid encrypted text received" errors and HashConnect compatibility issues. Platform now uses the proven working approach documented in changelog entry from June 25, 2025.
- June 26, 2025: **HASHCONNECT V3 MIGRATION ATTEMPTED**: Attempted migration to HashConnect v3 API but encountered persistent wallet pairing issues. findLocalWallets() returned empty objects and connectToExtension() failed to trigger wallet connections. Reverted to direct HashPack API approach that was previously working successfully.
